...
from .controllers import orders, order_details, recipes
from .dependencies.database import Base, engine, get_db
from .models.models import Order, OrderDetail, Sandwich, Resource, Recipe
...
def create_orderdetail(orderdetail: schemas.OrderDetailCreate, db: Session = Depends(get_db)):
    return order_details.create(db=db, detail=orderdetail)
...
def update_one_orderdetail(orderdetail_id: int, orderdetail: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    ...
    if orderdetail_db is None:
        raise HTTPException(status_code=404, detail="OrderDetail not found")
    return order_details.update(db=db, detail=orderdetail, detail_id=orderdetail_id)
...
def delete_one_orderdetail(orderdetail_id: int, db: Session = Depends(get_db)):
    ...
    if orderdetail is None:
        raise HTTPException(status_code=404, detail="OrderDetail not found")
    return order_details.delete(db=db, detail_id=orderdetail_id)